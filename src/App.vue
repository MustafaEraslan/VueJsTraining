<template>
  <div id="app">
    ...
    <div>
      <h3> Liste </h3>
      <div v-for="(item, i) of todos" 
        :key="i" 
        class="list-item"
        :title="'Oluşturma: ' + item.createdDate + '\nDeğiştirme: '+ item.modifiedDate">
        {{i+1}}. 
        <input v-model="item.isComplete" type="checkbox" @click="item.modifiedDate=new Date()" />
        <span 
          :class="[item.isComplete?'line-through':'']"
          >{{item.text}}</span>
